...
in the ``pandarm`` repo which gives the entire workflow,
but the discussion here will take things line-by-line with a sufficient
summary of the functionality.

Note that these code samples assume you have imported pandas and pandarm as follows::
...
but for now the initialization :py:meth:`pandarm.network.Network.__init__`
...
`downloaded here <https://s3-us-west-1.amazonaws.com/synthpop-data2/pandarm/osm_bayarea.h5>`__)::
...
id of the node and an x-y position.  Much like ``shapely``, ``pandarm`` is
...
Here is a link to the docs: :py:meth:`pandarm.network.Network.set_pois` ::
...
Here is a link to the docs: :py:meth:`pandarm.network.Network.nearest_pois`
...
Here is a link to the docs: :py:meth:`pandarm.network.Network.get_node_ids`
and :py:meth:`pandarm.network.Network.set`
